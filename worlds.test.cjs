...
test('unlit bridges cannot catch Pudim',()=>{const s=E.create(2),q=s.platforms.find(q=>q.kind==='bridge');Object.assign(s.player,{x:q.x+20,y:q.y-65,vy:100,grounded:false,support:-1});E.step(s,{},1/30);a.equal(s.player.grounded,false);a.ok(s.player.y+64>q.y);});
...
test('sky optional bow reached using updraft and closing umbrella',()=>{const s=E.create(1),n=s.platforms[15];place(s,11);s.player.x=s.platforms[11].x+s.platforms[11].w-55;for(let f=0;f<600&&!s.bow.taken;f++){const p=s.player,over=p.x>n.x;E.step(s,{right:p.x<n.x+45,left:p.x>n.x+65,jump:f===0,jumpHeld:!over||p.y+p.h>n.y},1/60);}a.equal(s.bow.taken,true);a.equal(s.rescues,0);});
test('night bow can be collected from optional high island',()=>{const s=E.create(2),n=s.platforms[17];s.bridgeTimers[1]=9;place(s,6);s.player.x=s.platforms[6].x+s.platforms[6].w-55;for(let f=0;f<240&&!s.bow.taken;f++){const p=s.player;E.step(s,{right:p.x<n.x+65,left:p.x>n.x+75,jump:f===0,jumpHeld:true},1/60);}a.equal(s.bow.taken,true);});
