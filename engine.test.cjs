...
test('fall preserves collected stars and bow',()=>{const s=E.create();place(s,9);tick(s);s.stars[0].taken=true;s.bow.taken=true;s.player.y=950;tick(s);assert.equal(s.player.support,9);assert.equal(s.rescues,1);assert.ok(s.lastFall.y>=950);assert.ok(s.lastFall.x>0);assert.ok(s.stars[0].taken&&s.bow.taken);});
