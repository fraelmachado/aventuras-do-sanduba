...
test('world has three distinct sections, five stars, special item and spaced flags',()=>{const s=E.create();assert.equal(s.sections.length,3);assert.ok(s.width>=6000);assert.equal(s.stars.length,5);assert.ok(s.platforms.filter(q=>q.checkpoint).length>=4);assert.ok(s.platforms.filter(q=>q.checkpoint).length<8);});
...
test('fall preserves collected stars and item',()=>{const s=E.create();place(s,9);tick(s);s.stars[0].taken=true;s.item.taken=true;s.player.y=950;tick(s);assert.equal(s.player.support,9);assert.equal(s.rescues,1);assert.ok(s.lastFall.y>=950);assert.ok(s.lastFall.x>0);assert.ok(s.stars[0].taken&&s.item.taken);});
...
test('arrival requires the special item and physical landing on final island',()=>{const s=E.create();s.player.x=s.goal.x;s.player.y=s.goal.y-70;s.player.vy=-10;tick(s);assert.equal(s.complete,false);place(s,s.goal.platform,s.goal.x-24);tick(s);assert.equal(s.complete,false);assert.ok(s.events.includes('needItem'));s.item.taken=true;tick(s);assert.equal(s.complete,true);assert.equal(s.stars.filter(q=>q.taken).length,0);});
...
test('spring gives access to optional high route and item',()=>{const s=E.create();place(s,15,s.platforms[15].x+s.platforms[15].w/2-24);let landed=false;for(let j=0;j<150;j++){const p=s.player;tick(s,{right:p.x<3500,left:p.x>3510,jumpHeld:true});if(p.grounded&&p.support===28){landed=true;break;}}assert.ok(landed);for(let j=0;j<100&&!s.item.taken;j++){const p=s.player;tick(s,{right:p.x<3710,left:p.x>3720,jump:j===0,jumpHeld:true});}assert.ok(s.item.taken);});
for(const fps of [30,60,120])test(`continuous garden traversal with spring and optional route at ${fps} FPS`,()=>{
 ...
 assert.ok(s.complete);assert.equal(s.rescues,0);assert.ok(s.item.taken);assert.equal(s.activeSection,2);
});
